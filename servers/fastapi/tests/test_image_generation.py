...
class TestImageGenerationService:
    ...
    def test_get_image_gen_func_defaults_to_gpt_image_1_5(self, mock_images_directory):
        """
        Test default OpenAI image provider when IMAGE_PROVIDER is not configured.
        """
        with patch.dict(os.environ, {}, clear=True):
            assert get_selected_image_provider() == ImageProvider.GPT_IMAGE_1_5
            service = ImageGenerationService(mock_images_directory)
            assert service.image_gen_func == service.generate_image_openai_gpt_image_1_5
                    ...
                    with patch('services.image_generation_service.is_pixabay_selected', return_value=False):
                        with patch('services.image_generation_service.is_gemini_flash_selected', return_value=False):
                            with patch('services.image_generation_service.is_dalle3_selected', return_value=True):
                                ...
                                async def mock_openai_generate(prompt, output_dir, model, quality):
    ...
    def test_gpt_image_1_5_falls_back_to_dalle3_on_rate_limit(self, mock_images_directory):
        """
        Test that gpt-image-1.5 falls back to DALL-E 3 only on rate limit errors.
        """
        async def run_test():
            service = ImageGenerationService(mock_images_directory)
            test_image_path = f"{mock_images_directory}/fallback_image.png"
            with open(test_image_path, "w") as f:
                f.write("fake image content")
            ...
            result = await service.generate_image_openai_gpt_image_1_5(
                "A product hero image", mock_images_directory
            )
...
class TestImageGenerationEndpoint:
    ...
    def app(self):
        ...
        app.dependency_overrides[get_async_session] = lambda: FakeAsyncSession()
        return app
