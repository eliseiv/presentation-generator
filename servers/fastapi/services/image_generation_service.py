...
def _is_openai_rate_limit_error(error: Exception) -> bool:
    if isinstance(error, RateLimitError):
        return True

    status_code = getattr(error, "status_code", None)
    error_code = getattr(error, "code", None)
    if not error_code and hasattr(error, "body") and isinstance(error.body, dict):
        error_body = error.body.get("error", {})
        if isinstance(error_body, dict):
            error_code = error_body.get("code") or error_body.get("type")

    return status_code == 429 or error_code in {"rate_limit_exceeded", "rate_limit"}
...
class ImageGenerationService:
    ...
    async def generate_image_openai(
        ...
        # Best-effort cost capture. Imported lazily so this service stays
        # importable when billing tables aren't migrated yet (e.g. fresh
        # checkout, unit-test that only touches image gen).
        try:
            from services.llm_cost_service import record_image_usage
            ...
            logger.exception("Failed to record image usage")
    ...
    async def _generate_image_google(
        ...
        try:
            ...
            await record_image_usage(provider="google", model=model, count=1)
        except Exception:
            logger.exception("Failed to record image usage")
